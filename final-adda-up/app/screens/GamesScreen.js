...
const GAMES = [
  ...
  { id: 'game2048', name: '2048', emoji: '🔢', color: '#ff9800', desc: '2048 tak pahuncho! (Swipe karo)', hasMultiplayer: false },
];
      ...
      <ScrollView contentContainerStyle={styles.grid}>
        {GAMES.map(game => (
          <TouchableOpacity key={game.id} style={[styles.gameCard, { borderColor: game.color }]}
            onPress={() => setActiveGame(game)} activeOpacity={0.85}>
            ...
            {game.hasMultiplayer && (
              <View style={[styles.multiTag, { backgroundColor: game.color + '33' }]}>
                <Text style={[styles.multiTagText, { color: game.color }]}>🌐 Online + 🤖 AI</Text>
              </View>
            )}
          </TouchableOpacity>
        ))}
      ...
      <Modal visible={!!activeGame} animationType="slide">
        <View style={{ flex: 1, backgroundColor: '#1a1a2e' }}>
          <View style={styles.gameHeader}>
            ...
            <Text style={styles.gameTitle}>{activeGame?.emoji} {activeGame?.name}</Text>
            <View style={{ width: 80 }} />
          </View>
...
const styles = StyleSheet.create({
  ...
  header: { padding: 16, paddingTop: 50, backgroundColor: '#16213e', borderBottomWidth: 1, borderBottomColor: '#0f3460' },
  ...
  gameCard: { width: '47%', backgroundColor: '#16213e', borderRadius: 16, padding: 18, alignItems: 'center', borderWidth: 1.5 },
  ...
  gameHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16, paddingTop: 50, backgroundColor: '#16213e', borderBottomWidth: 1, borderBottomColor: '#0f3460' },
  backBtn: { padding: 4 },
