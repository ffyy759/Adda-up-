...
export default function ProfileScreen({ navigation, route }) {
  ...
  const fetchLeaderPos = async () => {
    try {
      ...
      const sorted = snap.docs
        .map(d => ({ uid: d.id, coins: d.data().coins || 0 }))
  ...
  const pickDP = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission chahiye', 'Gallery access do!');
      return;
    }
    ...
    if (!result.canceled) {
      ...
      try {
        ...
        Alert.alert('Error', 'Upload nahi hua, dobara try karo!');
      }
  ...
  const pickPhoto = async () => {
    if ((userData?.photos || []).length >= 5) {
      Alert.alert('Limit!', 'Maximum 5 photos hi upload kar sakte ho!');
      return;
    }
    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    ...
    if (!result.canceled) {
      ...
      try {
        ...
        Alert.alert('Error', 'Upload nahi hua!');
      }
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalBox, { maxHeight: '70%' }]}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={{ color: '#888', textAlign: 'center', padding: 20 }}>Koi nahi hai abhi</Text>
              }
            />
      ...
      <ScrollView>
          ...
          {editing ? (
            <View style={styles.editForm}>
              ...
              <TextInput style={[styles.input, { height: 70, textAlignVertical: 'top' }]}
                value={bio} onChangeText={setBio} placeholder="Bio likho..."
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              {leaderPos && (
                <Text style={styles.leaderPosText}>🏆 Rank #{leaderPos} in UP</Text>
              )}
          ...
          <View style={styles.tabContent}>
            <View style={styles.allRanksContainer}>
              {RANKS.map((r, i) => {
                ...
                return (
                  <View key={i} style={[styles.rankBadge, !unlocked && styles.rankBadgeLocked,
                    { borderColor: unlocked ? r.color : '#333' }]}>
            ...
            {(userData?.badges || []).length === 0 && (
              <Text style={styles.emptyText}>Battles jeeto — badges milenge! 🔥</Text>
            )}
          </View>
        )}
        ...
        {activeTab === 'photos' && (
          <View style={styles.tabContent}>
            <View style={styles.photosGrid}>
              {(userData?.photos || []).map((url, i) => (
                <TouchableOpacity key={i} style={styles.photoItem}
                  onLongPress={() => isOwnProfile && deletePhoto(url)}>
            ...
            {isOwnProfile && (
              <Text style={styles.photoHint}>Photo pe long press karo delete karne ke liye</Text>
            )}
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: '#16213e', position: 'relative',
  },
  settingsBtn: {
  ...
  avatarCircle: {
    ...
    justifyContent: 'center', alignItems: 'center', borderWidth: 3, borderColor: '#ff6b3580',
  },
  avatarText: { fontSize: 38, fontWeight: 'bold', color: '#fff' },
  cameraIcon: {
    position: 'absolute', bottom: 0, right: 0,
    backgroundColor: '#16213e', borderRadius: 12,
    padding: 3, borderWidth: 1, borderColor: '#0f3460',
  },
  ...
  followBtn: {
    marginTop: 12, backgroundColor: '#ff6b35', paddingHorizontal: 28,
  ...
  statsRow: {
    ...
    borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#0f3460',
  },
  ...
  battleStat: {
    flex: 1, alignItems: 'center', backgroundColor: '#16213e',
  ...
  rankBadge: {
    alignItems: 'center', backgroundColor: '#16213e', borderRadius: 14,
    padding: 14, width: '30%', borderWidth: 1.5,
  },
  ...
  addPhotoBtn: {
    ...
    backgroundColor: '#16213e', borderWidth: 1.5, borderColor: '#0f3460',
  ...
  settingItem: {
    ...
    borderColor: '#0f3460', marginBottom: 10,
  },
  ...
  followUserRow: {
    ...
    padding: 12, borderBottomWidth: 1, borderColor: '#0f3460', gap: 12,
  },
